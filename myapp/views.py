...
def equity(request):
    ...
    calleven = LiveEquityResult.objects.annotate(odd=F('section') % 2).filter(odd=False).filter(strike__contains="call").order_by('section')  
    callodd = LiveEquityResult.objects.annotate(odd=F('section') % 2).filter(odd=True).filter(strike__contains="call").order_by('section') 
    puteven = LiveEquityResult.objects.annotate(odd=F('section') % 2).filter(odd=False).filter(strike__contains="put").order_by('section')  
    putodd = LiveEquityResult.objects.annotate(odd=F('section') % 2).filter(odd=True).filter(strike__contains="put").order_by('section') 
